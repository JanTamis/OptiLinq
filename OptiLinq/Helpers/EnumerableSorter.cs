	...
	where TQuery : struct, IOptiQuery<TElement, TEnumerable>
{
	...
	protected OrderedEnumerable(TQuery source) => _source = source;

	private int[] SortedMap(Buffer<TElement, TQuery, TEnumerable> buffer) => GetEnumerableSorter().Sort(buffer._items, buffer._count);

	private int[] SortedMap(Buffer<TElement, TQuery, TEnumerable> buffer, int minIdx, int maxIdx) =>
		GetEnumerableSorter().Sort(buffer._items, buffer._count, minIdx, maxIdx);

	public virtual IEnumerator<TElement> GetEnumerator()
	{
		var buffer = new Buffer<TElement, TQuery, TEnumerable>(_source);
		
		if (buffer._count > 0)
		{
			var map = SortedMap(buffer);
			for (var i = 0; i < buffer._count; i++)
			{
	...
	internal IEnumerator<TElement> GetEnumerator(int minIdx, int maxIdx)
	{
		...
		if (count > minIdx)
		{
			...
			{
				var map = SortedMap(buffer, minIdx, maxIdx);
				while (minIdx <= maxIdx)
				{
	...
	IOrderedEnumerable<TElement> IOrderedEnumerable<TElement>.CreateOrderedEnumerable<TKey>(Func<TElement, TKey> keySelector, IComparer<TKey>? comparer, bool descending) =>
		new OrderedEnumerable<TElement, TKey, TQuery, TEnumerable>(_source, keySelector, comparer, @descending, this);
	...
	where TQuery : struct, IOptiQuery<TElement, TEnumerable>
{
	...
	internal OrderedEnumerable(TQuery source, Func<TElement, TKey> keySelector, IComparer<TKey>? comparer, bool descending, OrderedEnumerable<TElement, TQuery, TEnumerable>? parent) :
		base(source)
	{
		_parent = parent;
	...
	internal override EnumerableSorter<TElement> GetEnumerableSorter(EnumerableSorter<TElement>? next)
	{
		...
		if (typeof(TKey) == typeof(string) && Equals(comparer, Comparer<string>.Default))
		{
	...
	where TQuery : struct, IOptiQuery<TElement, TEnumerable>
{
	...
	public OrderedImplicitlyStableEnumerable(TQuery source, bool descending) : base(source)
	{
		_descending = descending;
	}

	internal override CachingComparer<TElement> GetComparer(CachingComparer<TElement>? childComparer) =>
		childComparer == null ? new CachingComparer<TElement, TElement>(EnumerableSorter<TElement>.IdentityFunc, Comparer<TElement>.Default, _descending) : new CachingComparerWithChild<TElement, TElement>(EnumerableSorter<TElement>.IdentityFunc, Comparer<TElement>.Default, _descending, childComparer);
